...
// Linux propagates S_ISGID to a new subdirectory, the same class of
// divergence as the ZFS nfs4acl case ensureManagedDir was written for,
// and the only one reproducible in a unit test. Skips (rather than
// passing vacuously) when the kernel honours the mode request, since
// then there is nothing for enforcement to correct.
...
// site (Lstat, so a substituted symlink cannot answer for it). Reports
// a stat failure rather than aborting (C18), and its zero-value return
// on failure cannot equal managedDirStoredMode, so the caller's own
// comparison still ends red.
...
// TestEnsureManagedDir_verifiesTheModeItCreated pins the bug the replaced
// MkdirAll could not see: it asked for 0o755 and returned nil without
// reading back what the filesystem stored, so a widened directory raised
// no error. The widening here is real (see setgidParent), so the
// assertion can only hold if ensureManagedDir chmod'ed the handle and
// re-read it.
...
// TestNew_verifiesTheStoredModeOfTheBinDir pins the property at the call
// that actually creates bin/, the one PATH directory whose entries the
// probe executes: a mode wider than 0o755 lets any group member repoint
// an entry at an uninspected binary. New matters because it runs first —
// enforcing only in linkBin would leave the check permanently unreachable
// on a fresh volume.
...
// linkBin re-establishes bin/ on every publish, the path that runs when
// the directory was removed after New.
...
// TestExtractAndSwap_verifiesTheStoredModeOfThePublishedVersionTree pins
// why the staging directory is the right place to enforce: the publish
// is an os.Rename of that same inode, so its mode is the mode the
// published version tree ends up with — a group-writable version dir is
// the repoint attack one level below bin/. formatRaw keeps this a rename
// rather than a tar exec, whose own directory modes are a different
// concern.
...
// TestEnsureManagedDir_leavesAPreExistingDirectoryAlone pins the
// deliberate limit: a directory this call did not create is never
// chmod'ed, so an operator who tightened bin/ to 0o700 does not have it
// widened back on the next install. A pre-existing directory that is
// too OPEN is verifyRootIntegrity's subject instead.
...
// replaced os.MkdirAll had: EEXIST at an occupied name must not read as
// "already established" and hand a regular file back as a usable
// directory.
